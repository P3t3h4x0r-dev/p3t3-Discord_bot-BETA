...
class Logs(commands.Cog): #Define a classe.
    ...
    async def setupseguranca(self, ctx): #O usuário utiliza o comando "setuplog" (Cria o canal de 'logs') definindo aonde sera encaminhado os logs.
        guild = ctx.guild
        ...
        log_channel = await guild.create_text_channel('seguranca', overwrites=overwrites)  #Cria o canal com as permsissões no "overwrites".
        await log_channel.send('Canal de segurança criado com sucesso.') #Envia a mensagem de confirmação da criação do canal logs no "logs".